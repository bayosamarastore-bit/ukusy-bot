...
ext = os.path.splitext(f.filename)[1].lower() if ext not in (".jpg", ".jpeg", ".png", ".gif", ".webp", ".heic"):
...
def get_comments(place_id):
    ...
    return jsonify({"comments": comments})@app.route("/api/comments/", methods=["POST"])
...
лучшие кафе, добавь свои места, читай отзывы.\n\nОткрой приложение через меню → Открыть 
Гид",
                },
            )
        elif text == "/help":
